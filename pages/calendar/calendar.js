...
Page({
  data: {
    maxMonth: 7, //最多渲染月数
    'imgUrl_date': app.globalData.imgUrl + 'calendar@2x.png',
    dateList: [],
    systemInfo: {},
    ...
    dateNumber:1,
    sFtv:[
      {month:1,day:1,name:"元旦"},{month:2,day:14,name:"情人节"},
      ...
      {month:9,day:28,name:"孔子诞辰"},
      {month:10,day:1,name:"国庆节"},
      {month:10,day:6,name:"老人节"},
    ...
    if (curreInid == curreOutid) {//入住与离店是当月的情况
      for (let i = 0,l = dateList.length; i < l; i++) {
        if (dateList[i].id == curreInid) {
          ...
          for (let k = 0,len = days.length; k < len; k++) {
      ...
      for (let j = 0, l = dateList.length; j < l; j++) {
  ...
  createDateListData: function () {
    ...
    */
    ...
    for (let i = 0, l = dateList.length; i < l; i++){//加入公历节日
       for(let k = 0,l = sFtv.length; k < l; k++){
  ...
  onPressDate: function (e) {
    ...
    if (!this.data.markcheckInDate) {
      ...
      this.data.dateNumber = this.dayNumber(this.data.checkInDate,this.data.checkOutDate)
      ...
      wx.setStorage({
        ...
        data: {
          ...
          dateNumber: this.data.dateNumber
        }
      });
      wx.navigateBack({
